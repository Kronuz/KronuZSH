...
#                         (git, direnv, venv, cwd, command duration, ...).
...
# direnv / venv / keymap / cwd segments are small native pieces (prezto used its
...
# ---- direnv ----
# DIRENV_FILE is maintained by direnv's own prompt hook. The hook runs before
# kz_prompt_precmd, so this stays current without invoking direnv again on the prompt
# path. direnv also sets DIRENV_FILE for a blocked/failed rc file; this deliberately
# represents a detected direnv context, not its approval state.
typeset -g _kz_prompt_direnv_root=''
function _kz_direnv_segment {
  kz[direnv.file]=''
  kz[direnv.root]=''
  _kz_prompt_direnv_root=''
  [[ -n "${DIRENV_FILE-}" ]] || return 0

  local root="${DIRENV_FILE:h:t}"
  [[ -n "$root" ]] || root='/'
  kz[direnv.file]="$DIRENV_FILE"
  kz[direnv.root]="$root"
  _kz_prompt_direnv_root="${root//\%/%%}"
}
...
function kz_prompt_reset_skin {
  local -a skin_vars=(
    ...
    KZ_PROMPT_GIT KZ_PROMPT_DIRENV KZ_PROMPT_PWD KZ_PROMPT_VENV KZ_PROMPT_CARET
    KZ_PROMPT_STATUS
  )
  unset ${^skin_vars}
}
...
function kz_prompt_setup {
  ...
  DEFAULT_KZ_PROMPT_DIRENV='${_kz_prompt_direnv_root:+" ${_kz_sem[info]}env${kz[RESET]}:${_kz_sem[direnv]}${_kz_prompt_direnv_root}${kz[RESET]}"}'
  ...
  kz[direnv]='${(e)KZ_PROMPT_DIRENV-$DEFAULT_KZ_PROMPT_DIRENV}'
  ...
  # array -- os err info context etctl git direnv venv jobs nl time pwd caret
  # transient_caret
  ...
  DEFAULT_KZ_PROMPT_PROMPT='$kz[err] $kz[info]$kz[context]$kz[etctl]$kz[git]$kz[direnv]$kz[venv]$kz[jobs]$kz[nl]$kz[time] $kz[pwd] $kz[caret] '
