...
typeset -a _kz_run_help_files
_kz_run_help_files=(${^fpath}/run-help(N[1]))
unalias run-help 2>/dev/null
if (( $#_kz_run_help_files )); then
  autoload -Uz run-help

  for _kz_run_help in run-help-{git,ip,openssl,sudo}; do
    _kz_run_help_files=(${^fpath}/$_kz_run_help(N[1]))
    (( $#_kz_run_help_files )) && autoload -Uz "$_kz_run_help"
  done
  unset _kz_run_help
else
  function run-help { command man "$@" }
fi
unset _kz_run_help_files
...
function help {
  ...
  if (( $# == 1 && $+_kz_help_native[$1] )); then
    ...
    ( unfunction "$1" 2>/dev/null; run-help "${_kz_help_native[$1]}" )
