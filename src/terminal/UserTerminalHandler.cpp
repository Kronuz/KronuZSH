...
#define BUF_SIZE (16 * 1024)
  ...
  // input write: the old blocking `RawSocketUtils::writeAll(masterFd, ...)` did,
  // and a large burst of input (e.g. a pasted heredoc) echoes back, fills the
  // pty output buffer, stalls the shell, and the shell then stops reading input
  // -- so the write never completes and we also stop draining output: a deadlock
  // that wedges the session past ~one pty buffer of input.  Instead we buffer
  // pending input, drain it to the pty whenever it is writable, and keep reading
  // output every iteration.  When the buffer fills we stop reading more input
  // from the router, so backpressure reaches the client.
  string pendingInput;
  ...
  while (true) {
    ...
    // Stop pulling more input from the router once the pty-input buffer is full,
    // so backpressure reaches the client instead of buffering without bound.
    ...
    try {
      ...
      if (!pendingInput.empty()) {
        ...
        } else if (rc < 0 && writeErrno != EAGAIN && writeErrno != EWOULDBLOCK) {
