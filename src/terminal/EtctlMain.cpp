 ...
 * interrupt, eof, expect) are ergonomic sugar composed from it.
 */
#include <algorithm>
#include <csignal>
#include <fstream>
 ...
 * shell on the far side; full-screen output is for read/observe.  If `bodyOut` is
 * non-null the body is captured there; otherwise it is written to stdout.  (Validated
 * against a live et session, not the in-process echo harness, which does not execute
 * commands.)
 */
int runCommand(const string& name, const string& command, double timeoutSec,
  ...
  // Echo the start marker, run the command, echo the end marker + $?.  The
  // command is wrapped in a brace group so a multi-line body runs as one unit:
  // the open '{' leaves the first input line incomplete, so even a line-editing
  // shell (zsh) waits for the closing '}' instead of submitting the first
  // statement early.  '{ ...; }' runs in the current shell, so cd/export persist.
  string framed =
      "echo " + mark + "; { " + command + "\n}; echo " + mark + ":$?\n";
