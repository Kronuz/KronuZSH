...
// and cannot reproduce the input-write deadlock -- which is exactly why that bug
// went unnoticed.  `cat` echoes input back byte-for-byte; the pty is raw +
...
// that hung past ~one pty buffer of input.  This test times out on that code and
// passes with the buffered non-blocking input drain.
void largeInputNoDeadlockTest(
    shared_ptr<PipeSocketHandler> routerSocketHandler,
    SocketEndpoint serverEndpoint, shared_ptr<SocketHandler> clientSocketHandler,
    shared_ptr<SocketHandler> clientPipeSocketHandler,
  ...
  // deadlock the pre-fix handler -- but small enough not to stress the pipe-based
  // test transport.
  ...
  // Read the echo back under a watchdog; the deadlocking code never delivers it.
  ...
  bool completed = doneFuture.wait_for(std::chrono::seconds(20)) ==
