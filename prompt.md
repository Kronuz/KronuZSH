...
Every option is an ordinary Zsh parameter normally set in `~/.zshrc.local`. A project's
approved `.autoenv.zsh` can scope an override with `autostash`; stash first and assign
separately so the prompt parameter stays shell-local. Do not `export` prompt options:
they affect only the current shell and its prompt. All of them are optional; out of the
box the prompt auto-detects your terminal, OS, and session. Nerd Font glyphs are enabled
by default, with a plain-Unicode set available for terminals using another font.
...
env:project   autoenv      project root containing the loaded .autoenv.zsh
...
autoenv context, no venv) simply don't render, so the prompt stays as short as the
moment allows.
...
| `autoenv`                    | aqua                          | zsh-autoenv project root                             |
...
terminal mark. Set `KZ_PROMPT_STATUS=0` to retain the older live-only
...
When zsh-autoenv has approved and sourced a `.autoenv.zsh`, the prompt shows
`env:<root>` beside the Git and virtualenv segments. The root comes from
zsh-autoenv's in-shell loaded-file stack, so entering a subdirectory keeps the
environment boundary visible without running another command from the prompt.

Declined files never enter that stack, so the badge means “autoenv sourced,” not merely
“file detected.” Set `KZ_PROMPT_AUTOENV=''` to hide it, or use
`$kz[autoenv.file]` and `$kz[autoenv.root]` in a custom skin.
...
| `KZ_PROMPT_STATUS`           | `1`     | Keep a failed exit status and/or slow-command duration in scrollback when the next command collapses, or show it in the static prompt when transience is disabled; `0`/`no`/`off`/`false` makes it live-only with transience and hides it without transience.                                                                           |
...
| `KZ_PROMPT_AUTOENV`   | loaded zsh-autoenv project root (`env:<root>`); `''` hides it                      |
...
| `KZ_PROMPT_<SEGMENT>`            | built in         | Replace one complete segment or outcome item. Names: `OS`, `ERR`, `ERROR`, `DURATION`, `USER`, `HOST`, `IP`, `TIME`, `PWD`, `GIT`, `AUTOENV`, `VENV`, `JOBS`, `CONTEXT`, `ETCTL`, `VIM`, `EMACS`, `OVERWRITE`, `PROMPT`; see [Replacing a whole segment](#replacing-a-whole-segment).                                                                                                               |
...
| `KZ_PROMPT_STATUS`               | `1`              | Keep the previous failed status and/or duration above the next collapsed command, or show it in the static prompt when transience is disabled; false values make it live-only with transience and hide it without transience.                                                                                                                                                                        |
...
| `_autoenv_stack_entered`             | (zsh-autoenv)    | In-shell stack of approved, loaded `.autoenv.zsh` files; drives `env:<root>` and the normalized `$kz[autoenv.*]` state.                                                                                                                                                                                                                                                                                |
