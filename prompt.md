...
# Past commands collapse to a timestamp, path, and caret with their original colors.
# Flatten them to ANSI 8 instead, or turn the whole transient behavior off:
KZ_PROMPT_TRANSIENT_STYLE=darkgray
...
- **Plain Unicode**: BMP symbols that render in any normal font. Switch to it with
  `KZ_PROMPT_NERD_FONT=0` (also accepts `no`, `off`, `false`). It's also forced
  automatically on a `dumb`/unknown terminal, where Nerd Font glyphs would be tofu.

Override any single glyph in either set with `KZ_PROMPT_GLYPH_<NAME>`: set it to
...
and omitted for normal ones; override it per glyph with `KZ_PROMPT_GLYPH_PAD_<NAME>`.
Set it to `''` to hug tight, or to a space, a non-breaking space (`$'\u00a0'`), or any
string to tune the spacing for your font (e.g. `KZ_PROMPT_GLYPH_PAD_UNTRACKED=''`).
...
The conventional ANSI neutral names follow the terminal theme:

| Name(s) | Value |
| --- | --- |
| `gray` / `darkgray` / `white` | ANSI 7 / 8 / 15 |
| `lightgray` | `#c6c6c6` |

There are no `grey` or `dimgray` aliases; use the American names or `black`.
The retired `neutral`, `muted`, `bright`, `kronuzblue`, and British `grey` names are
reserved and ignored rather than recreated as custom hues.
...
rest of the 16 ANSI names, and non-retired custom names are valid too. This pins that hue across
the whole prompt (everything built on it) and tells `dimmed` its real RGB, which is the
...
| `pwd`                        | blue; medium spring green over SSH; violet in a container (red as root) | working directory |
...
| `caret_past`                 | bold gray                     | the collapsed transient caret                        |
...
After a command runs longer than `KZ_PROMPT_CMD_DURATION_MIN` seconds
...
(`~/.config/KronuZSH/integrations/bat`). `KZ_PROMPT_PWD_STYLE` shortens it:
...
`KZ_PROMPT_PWD` instead — see [Replacing a whole segment](#replacing-a-whole-segment).
...
The collapsed path reuses your `KZ_PROMPT_PWD_STYLE` (so `short`/`base` shorten it
...
restyled together by `KZ_PROMPT_TRANSIENT_STYLE`.
...
[16:25:58] ~/project/src ❯ ./run --watch  ← after Enter, the old prompt collapses
3.4s         ← the completed command was slow; visible in the new live prompt
● kronuz at host (10.0.0.5)  ⎇ main
[16:26:02] ~/project/src ❯❯❯
```

These variables control it (the palette knobs `dimmed` relies on are described under the
...
| `KZ_PROMPT_TRANSIENT_STYLE`  | `original` | How the collapsed line — the pwd, caret, and the just-run **command** — is restyled: `original`, `dimmed`, or any palette hue (`darkgray`, `gray`, `pink`, a custom name, ...). Unknown values report an error and fall back to `original`.                                                                                                  |
| `KZ_PROMPT_TRANSIENT_DIM`    | `0.7`   | For `dimmed`: darkness factor, `0` = black, `1` = unchanged. Lower is darker.                                                                                                                                                                                                                                                           |

The style modes:

- **`dimmed`** keeps the command's own syntax colors but darkens them, so the line
  ...
  gets each from its `KZ_PROMPT_PALETTE_<NAME>` override if you set one, else from
  an on-disk cache, else a one-time **OSC 4** query of the terminal (cached afterward
  for `KZ_PROMPT_PALETTE_TTL`); if nothing answers it falls back to the xterm
  defaults. Over a remote shell (e.g. SSH or Eternal Terminal) the query round-trip is
  network-bound, so the cache and a generous `KZ_PROMPT_PALETTE_TIMEOUT` matter; if
  ...
  KZ_PROMPT_PALETTE_BLACK='#000000'    KZ_PROMPT_PALETTE_DARKGRAY='#686868'
  ...
  KZ_PROMPT_PALETTE_GRAY='#f1f1f0'     KZ_PROMPT_PALETTE_WHITE='#eff0eb'
  ```
- **Any palette hue** repaints the collapsed prompt and command with that one flat
  color. `darkgray` (ANSI 8) and `gray` (ANSI 7) are the natural subdued choices,
  but `pink`, `red`, or a custom hue such as `ocean` work identically. A live
  `KZ_PROMPT_PALETTE_<NAME>` override updates both the
  prompt escape and zsh's native `region_highlight` spec.
- **`original`** leaves the syntax colors untouched.
...
`KZ_PROMPT_TERMINAL_INTEGRATION` is not disabled and test outside ordinary
...
`KZ_PROMPT_<SEGMENT>`. The value is a prompt string (zsh `%`-escapes and
...
KZ_PROMPT_TIME='[%D{%H:%M:%S}]'

# Just the basename of the cwd (or simpler: KZ_PROMPT_PWD_STYLE=base; for the
# shortest-unique-prefix ~/.c/K/i/bat, KZ_PROMPT_PWD_STYLE=short):
KZ_PROMPT_PWD='%1~'
...
| ------------------------------------ | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `KZ_PROMPT_NERD_FONT`            | `1`              | `0`/`no`/`off`/`false` switches to the plain-Unicode glyph set.                                                                                                                                                                                                                                                                                                                                      |
| `KZ_PROMPT_GLYPH_<NAME>`         | per glyph        | Override one glyph; `''` hides it. All names are in the [glyph table](#glyphs).                                                                                                                                                                                                                                                                                                                      |
| `KZ_PROMPT_GLYPH_PAD_<NAME>`     | per glyph        | Override a glyph's trailing (right-hand) pad; `''` hugs tight, a space / `$'\u00a0'` / any string tunes it for your font.                                                                                                                                                                                                                                                                            |
...
| `KZ_PROMPT_PALETTE_<NAME>`       | terminal palette | Define or override a base hue with `#RRGGBB` or a 0–255 index. Built-in names include `BLACK`, `RED`, `GREEN`, `YELLOW`, `BLUE`, `MAGENTA`, `CYAN`, `GRAY`, `DARKGRAY`, `LIGHTRED`, `LIGHTGREEN`, `LIGHTYELLOW`, `LIGHTBLUE`, `LIGHTMAGENTA`, `LIGHTCYAN`, `WHITE`, and fixed `LIGHTGRAY`; non-retired custom names work too and become `$kz[FG.<name>]`, `$kz[BG.<name>]`, and native `$kz[HL.<name>]` ZLE specs. This changes display colors, flat transient command colors, and the RGB used by `dimmed`. |
...
| `KZ_PROMPT_TRANSIENT_STYLE`      | `original`       | Restyle of the collapsed line (pwd, caret, command): `original`, `dimmed`, or any palette hue (`darkgray`, `gray`, `pink`, custom names, ...). Unknown values report an error and use `original`.                                                                                                                                                                                                      |
...
shell after changing `KZ_PROMPT_PALETTE_*`, `KZ_PROMPT_PALETTE_TTL`, or
`KZ_PROMPT_PALETTE_TIMEOUT` when you need the dimmed colors to be recalculated.
