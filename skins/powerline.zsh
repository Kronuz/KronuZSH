...
# from ${bcol[...]} (the derived background palette), foregrounds from ${fcol[...]}.
PROMPT_KRONUZ_GIT='${_prompt_kronuz_git_branch:+${bcol[green]}${fcol[blue]}'$'\ue0b0''${fcol[gray]} '$'\ue0a0'' ${_prompt_kronuz_git_branch} }'
PROMPT_KRONUZ_PS1='${bcol[gray]}${fcol[white]} %n@%m ${fcol[darkgrey]}${bcol[blue]}'$'\ue0b0''${fcol[white]} %~ $kronuz[git]%k%F{${${_prompt_kronuz_git_branch:+2}:-4}}'$'\ue0b0''%f '
PROMPT_KRONUZ_RPS1=''
