...
# (os err info context etctl git autoenv venv jobs nl time pwd caret overwrite vim emacs).
#
# Live prompt. Line 1: error dot, user@host, ssh/container badge, etctl session, git,
# autoenv, venv, jobs. Then a newline, and line 2: time, path, caret.
KZ_PROMPT_PROMPT='$kz[err] $kz[info]$kz[context]$kz[etctl]$kz[git]$kz[autoenv]$kz[venv]$kz[jobs]$kz[nl]$kz[time] $kz[pwd] $kz[caret] '
